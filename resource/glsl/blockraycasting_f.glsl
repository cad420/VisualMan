...
layout( binding = 5, rgba32f ) uniform volatile image2D texNeuroPos;
uniform float step;
// illumination params
...
void main()
{
	...
	vec3 neuroPos = imageLoad( texNeuroPos, ivec2( gl_FragCoord ) ).xyz;
	//imageStore( entryPos, ivec2( gl_FragCoord ), vec4( rayEnd, 0.0 ) );	 // Terminating flag

	//fragColor = vec4( neuroPos, 1.0 );
	//return;

	float neurodis = distance( neuroPos, rayStart );
	//fragColor = vec4( neurodis, 0 , 0, 1.0 );
	//return;
	bool neuro = !( neuroPos.x == 0.0 && neuroPos.y == 0.0 && neuroPos.z == 0.0 );
	...
	for ( int i = 0; i < 10000; ++i ) {
		...
		//float dist = stepTable[ curLod ] * ( float( i ) + 0.5 );
		if ( neuro && ( length( neuroPos - samplePoint ) < 0.01 ) ) {
			if ( color.a > 0.7 ) color.a = 0.7;
			color = color + vec4( 0.0, 1.0, 0.0, 1.0 ) * ( 1.0 - color.a );
			break;
		}
...
*/
		//if ( color.a > 0.99 )
		if ( !neuro && color.a > 0.99 ) {
			break;
		}/* else if ( neuro && color.a > 0.99 ) {
			color = color * 0.5 + vec4( 0.0, 1, 0.0, 1.0 ) * 0.5;
			color.a = 1.0;
		}*/
	}
