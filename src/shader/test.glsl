            ...
            vec2 passbox(ivec3 start, int k){   
               float lowmax=value0; float highmin=float(1<<14); float tf1; float tf2; float tf3; int p=0;    
               for (int ii=0;ii<3;ii++){     
                   if (dir[ii]!=0.0){      
                      tf1=(float(start[ii])*bSpan[ii]-camera[ii])/dir[ii];    
                       tf2=((float(start[ii]+POW(k)))*bSpan[ii]-camera[ii])/dir[ii];   
                       if (tf1>tf2) {tf3=tf1;tf1=tf2;tf2=tf3;}   
            ...
            ivec4 pass8(ivec3 start, int k){          
               start=start*POW(k); vec2 pass0; pass0=passbox(start,k);      
               ivec4 res=ivec4(-1,-1,-1,-1);      
               if ((pass0[1]-pass0[0])<=0.0) return res;      
               vec3 entry=camera+dir*pass0[0];   
               vec3 passplate; ivec3 plates; int p=0; float kp;   
               ...
               for (int ii=0;ii<3;ii++){     
                  if (dir[ii]==0.0) continue;     
                   ...
                   if (kp>=pass0[1] || kp<pass0[0]) continue;    
                    passplate[p]=kp; plates[p]=ii; p++;      
                }  
                float tf; int ti;     
                for (int ii=0;ii<p;ii++){    
                    for (int ij=ii+1;ij<p;ij++){      
                        if (passplate[ii]>passplate[ij]){  
                            tf=passplate[ii];passplate[ii]=passplate[ij];passplate[ij]=tf;    
                            ti=plates[ii];plates[ii]=plates[ij];plates[ij]=ti;    
                        }     
                    }     
                }     
                for (int ii=0;ii<p;ii++) {res[ii+1]=res[ii]^POW(plates[ii]); } return res;  
             }  
             void main(void){     
                value0=texture(terminal, vec2((float(gl_GlobalInvocationID.x)+0.5)/512.0,(float(gl_GlobalInvocationID.y)+0.5)/512.0))[0];   
             	...
             	dir=dir/length(dir);     
                ivec3 sv[36]; int sp[36]; int sl=1; int ii; ivec4 res[TAILLONG]; int resp=0;    
                for(int iii=0;iii<32;iii++) res[iii]=ivec4(-1,-1,-1,-1);   
                sv[0]=ivec3(0,0,0); sp[0]=rootpow;      
                while(sl>0){     
                    sl--; ivec3 tv=sv[sl]; int tp=sp[sl];    
